...
inline half4 LightingStandard (SurfaceOutputStandard si, half3 viewDir, UnityGI gi)
{
    ...
    return aUnityColor(s, gi, si.Shadow);
}
...
{
    ASurface s = aStandardSurface(si, viewDir);
    return aUnityGbuffer(s, gi, outGBuffer0, outGBuffer1, outGBuffer2);
}
	...
	inout UnityGI gi)
{
    // Alloy
#if defined(UNITY_PASS_DEFERRED) && UNITY_ENABLE_REFLECTION_BUFFERS
    gi = UnityGlobalIllumination(data, 1.0h, s.Normal);
#else
    Unity_GlossyEnvironmentData g = UnityGlossyEnvironmentSetup(s.Smoothness, data.worldViewDir, s.Normal, lerp(unity_ColorSpaceDielectricSpec.rgb, s.Albedo, s.Metallic));
    gi = UnityGlobalIllumination(data, 1.0h, s.Normal, g);
#endif
}
