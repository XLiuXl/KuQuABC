...
namespace ThirdPersonCamera
{
    ...
    public class CameraController : MonoBehaviour
    {
        ...
        private System.Collections.Generic.List<RayCastWithMags> rcms;
        #endregion
		...
		void Start(){
			StartCoroutine (WaitFind(3f));
		}
        

		IEnumerator WaitFind(float t){
            ...
            //opt
            if (!this.GetComponent<TNObject>().isMine)
            {
                this.GetComponent<Camera>().enabled = false;
                Debug.Log("--------------------------------------CameraController Disable Camera---------------------");
            }
		}
        ...
        void Update()
        {
            ...
            if (hits.Length > 0)
            {
                rcms = new System.Collections.Generic.List<RayCastWithMags>(hits.Length);
