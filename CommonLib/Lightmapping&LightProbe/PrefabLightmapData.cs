...
public class PrefabLightmapData : MonoBehaviour
{
    ...
    public static bool isBakedCompleted = false;
    [System.Serializable]
  ...
  public  static void UpdateLightmaps()
    ...
    public static void SaveReflectionProbes()
    {
       var probes = FindObjectsOfType<ReflectionProbe>();

        for (int i = 0; i < probes.Length; i++) {
            if (probes[i].gameObject.GetComponent<ReflectionProbeParams>() == null)
            {
                var t = probes[i].gameObject.AddComponent<ReflectionProbeParams>();
                t.bakedTexture = probes[i].bakedTexture;
            }
            else {
                var t = probes[i].gameObject.GetComponent<ReflectionProbeParams>();
                t.bakedTexture = probes[i].bakedTexture;
            }
        }
    }

    public static void SaveLightProbeGroup()
    {
        ...
        var lightProbeGroup = FindObjectsOfType<LightProbeGroup>();

        var newSphericalHarmonicsList = new List<LightProbeParams.SphericalHarmonics>();

        for (int i = 0; i < bakedProbes.Length; i++)
        {
            var SHCoeff = new LightProbeParams.SphericalHarmonics();
            ...
            for (int j = 0; j < 3; j++)
            {
                ...
                for (int k = 0; k < 9; k++)
                {
                    SHCoeff.coefficients[j * 9 + k] = bakedProbes[i][j, k];
                }
            }
        ...
        for (int i = 0; i < lightProbeGroup.Length; i++)
        {
            if (lightProbeGroup[i].gameObject.GetComponent<LightProbeParams>() == null)
            {
                var t = lightProbeGroup[i].gameObject.AddComponent<LightProbeParams>();
                t.probePos = probePositions;
                t.bakedProbes = newSphericalHarmonicsList.ToArray();
            }
            else
            {
                var t = lightProbeGroup[i].gameObject.GetComponent<LightProbeParams>();
                t.probePos = probePositions;
                t.bakedProbes = newSphericalHarmonicsList.ToArray();
            }
        }
    ...
    public static void AddLightMapCmp()
    {
        var t = Selection.activeGameObject;
        if (t.GetComponent<PrefabLightmapData>() == null)
            t.AddComponent<PrefabLightmapData>();
    }
   ...
   isBakedCompleted= Lightmapping.Bake();
        ...
        foreach (var instance in prefabs)
        {
            if (instance.GetComponents<PrefabLightmapData>().Length > 1)
            {
                var t = instance.GetComponents<PrefabLightmapData>();
                DestroyImmediate(t[0]);
            }
        }


            //foreach (var p in FindObjectsOfType<LightProbeGroup>())
            //    if (p!=null&&p.GetComponent<LightProbeParams>() == null) {
            //        p.gameObject.AddComponent<LightProbeParams>();
            //    }

            //foreach (var p in FindObjectsOfType<ReflectionProbeParams>())
            //    if (p != null&&p.GetComponent<ReflectionProbeParams>() == null)
            //    {
            //        p.gameObject.AddComponent<ReflectionProbeParams>();
            //    }

            Debug.Log("Update to prefab lightmaps|ReflectionProbe|LightProbeGroup finished");
    }
