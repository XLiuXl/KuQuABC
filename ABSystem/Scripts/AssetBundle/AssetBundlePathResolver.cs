...
namespace ABSystem
{
    ...
    public class AssetBundlePathResolver
    {
        ...
        /// AB 保存的路径[通过数据库查询合成]
        /// </summary>
        public virtual string BundleSaveDirName { get { return PlayerPrefs.GetString("BundleDir")+ "/AssetBundles"; } }
        ...
        public string BundleSavePath {
            get {
                ...
                string subPath = PlayerProfile.VLevel + "/" + PlayerProfile.VScene + "/" + PlayerProfile.VTitle + "/" + BundleSaveDirName;

                subPath=subPath.Substring(2);
                
                string paths = path + "/Data" + subPath;

                Debug.Log("SavePath:"+paths);
        ...
        public virtual string GetBundleSourceFile(string path, bool forWWW = true)
        {
            ...
            string paths = System.Environment.CurrentDirectory;
            paths = paths.Replace("\\", "/");

            string abDir = paths + "/Data/" + PlayerProfile.VLevel + "/" + PlayerProfile.VScene + "/" + PlayerProfile.VTitle + "/AssetBundles";


#if UNITY_EDITOR || UNITY_STANDALONE
            if (forWWW)
                //filePath = string.Format("file://{0}/StreamingAssets/{1}/{2}", Application.dataPath, BundleSaveDirName, path);
                filePath = string.Format("file://{0}/{1}", abDir, path);
               
            else
                filePath = string.Format("{0}/{1}", abDir, path);
        ...
        public virtual string BundleCacheDir
        {
            get
            {
                if (cacheDir == null)
                {
#if UNITY_EDITOR||UNITY_STANDALONE//Standalone 版本缓存
                    string dir = BundleSaveDirName;
#else
					string dir = BundleSaveDirName;
#endif
